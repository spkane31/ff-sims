...
install_units() {
  ...
  for unit in ff-sims-worker.service ff-sims-espn-worker.service ff-sims-deploy.service ff-sims-deploy.timer ff-sims-discovery.service ff-sims-discovery.timer ff-sims-lifetime-counts.service ff-sims-lifetime-counts.timer; do
...
Logs:
  journalctl -u ff-sims-worker -f      # Go Temporal worker logs (drafts, transactions, etc.)
...
main() {
  ...
  if ensure_env_file; then
    systemctl enable ff-sims-worker.service ff-sims-espn-worker.service ff-sims-deploy.timer ff-sims-discovery.timer ff-sims-lifetime-counts.timer
    systemctl start ff-sims-worker.service ff-sims-espn-worker.service ff-sims-deploy.timer ff-sims-discovery.timer ff-sims-lifetime-counts.timer
  else

...
export const getTeams = async (year: number): Promise<TeamData[]> => {
  const query = `
    SELECT
      teams.espn_id AS id,
  ...
  const teamData: QueryResult = await client.query(query, [year]);
  console.log(
    `[INFO] received ${teamData.rows.length} rows from getTeams query`
  );

  const respIDAsInt: TeamData[] = teamData.rows.map((row) => {
    return {
      id: parseInt(row.id),
      owner: row.owner,
      averageScore:
  ...
  const leagueData: QueryResult = await client.query(queryLeague, [year]);
  console.log(
...
export const getSchedule = async (year: number): Promise<Game[][]> => {
  const client = await pool.connect();
  const resp: QueryResult = await client.query(scheduleQuery);
  console.log(`[INFO] received ${resp.rows.length} rows from schedule query`);
  client.release();

  const parsedResponse: Game[] = resp.rows.map((row) => {
    return {
...
export const logRequest = async (
  ...
  try {
    const client = await pool.connect();
    const query = `
      INSERT INTO requests (endpoint, method, user_agent, runtime_ms, status_code, timestamp)
      VALUES ($1, $2, $3, $4, $5, $6);`;
    const values = [
      req.url,
      req.method,

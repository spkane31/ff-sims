...
// RecalculateExpectedWins queues a recalculation job for expected wins
// POST /api/v1/leagues/{id}/expected-wins/recalculate/{year}
func RecalculateExpectedWins(c *gin.Context) {
	...
	go func() {
		slog.Info("Starting expected wins recalculation", "league", leagueID, "year", year)

		// Recalculate weekly data
		err := simulation.RecalculateWeeklyExpectedWins(leagueID, year)
		if err != nil {
			slog.Error("Failed to recalculate weekly expected wins", "error", err, "league", leagueID, "year", year)
			return
		}

		// Recalculate season data if regular season is complete
		if simulation.IsRegularSeasonComplete(database.DB, leagueID, year) {
			err := simulation.RecalculateSeasonExpectedWins(leagueID, year)
			if err != nil {
				slog.Error("Failed to recalculate season expected wins", "error", err, "league", leagueID, "year", year)
				return
			}
		}

		slog.Info("Completed expected wins recalculation", "league", leagueID, "year", year)
	}()

	c.JSON(http.StatusAccepted, gin.H{"message": "Recalculation started"})
}

// ProcessWeek manually processes a specific week's expected wins
// POST /api/v1/leagues/{id}/expected-wins/process/{year}/{week}
func ProcessWeek(c *gin.Context) {
	leagueID, err := parseUintParam(c, "id")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid league ID"})
		return
	...
	week, err := parseUintParam(c, "week")
	if err != nil {
	...
	if err != nil {
		slog.Error("Failed to process week", "error", err, "league", leagueID, "year", year, "week", week)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to process week"})
		return
	}

	c.JSON(http.StatusOK, gin.H{"message": "Week processed successfully"})
}

// FinalizeSeasonExpectedWins finalizes season expected wins
// POST /api/v1/leagues/{id}/expected-wins/finalize/{year}
func FinalizeSeasonExpectedWinsHandler(c *gin.Context) {
	leagueID, err := parseUintParam(c, "id")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid league ID"})
		return
	...
	if err != nil {
		slog.Error("Failed to finalize season", "error", err, "league", leagueID, "year", year)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to finalize season"})
		return
	}

	c.JSON(http.StatusOK, gin.H{"message": "Season finalized successfully"})
}

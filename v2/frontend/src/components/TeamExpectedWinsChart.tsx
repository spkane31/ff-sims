...
function prepareTeamChartData(progressionData: WeeklyExpectedWins[]): ChartDataPoint[] {
...
export default function TeamExpectedWinsChart({ 
  ...
  const chartData = useMemo(() => prepareTeamChartData(progressionData), [progressionData]);
  const maxWeek = useMemo(() => {
    return chartData.reduce((max, week) => Math.max(max, week.week), 0);
  }, [chartData]);

  const maxWins = useMemo(() => {
    return chartData.reduce((max, week) => Math.max(max, week.actualWins, week.expectedWins), 0);
  }, [chartData]);

  const teamName = progressionData.length > 0 ? progressionData[0].team?.name || 'Team' : 'Team';
  ...
  if (chartData.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-700 p-6 rounded-lg shadow-md">
        <h3 className="text-lg font-semibold mb-4">{teamName} Expected vs Actual Wins - {currentYear}</h3>
  ...
  // Generate paths
  const expectedPath = chartData.map((point, index) => {
    const x = xScale(point.week);
    const y = yScale(point.expectedWins);
    return index === 0 ? `M ${x} ${y}` : `L ${x} ${y}`;
  }).join(' ');

  const actualPath = chartData.map((point, index) => {
    const x = xScale(point.week);
  ...
  return (
    <div className="bg-white dark:bg-gray-700 p-6 rounded-lg shadow-md">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
        Track this team's actual wins compared to expected wins throughout the season
      </p>
      ...
      <div className="overflow-x-auto">
        <svg width={chartWidth} height={chartHeight} className="border rounded">
          <g transform={`translate(${margin.left}, ${margin.top})`}>
            ...
            {chartData.map((point) => (
              <g key={`points-${point.week}`}>
                ...
                >
                  <title>
                    Week {point.week}: {point.expectedWins.toFixed(2)} expected wins
                  </title>
                </circle>
                ...
                >
                  <title>
                    Week {point.week}: {point.actualWins} actual wins (Luck: {point.winLuck > 0 ? '+' : ''}{point.winLuck.toFixed(2)})
                  </title>
                </circle>
                ...
                >
                  <title>
                    ...
                    (Win Probability: {(point.weeklyWinProbability * 100).toFixed(1)}%)
                    Point Differential: {point.pointDifferential > 0 ? '+' : ''}{point.pointDifferential.toFixed(2)}
                  </title>
                </circle>
              </g>
            ))}
          ...
          <g transform={`translate(${chartWidth - 180}, ${margin.top + 10})`}>
            <rect x={0} y={0} width={170} height={100} fill="#F9FAFB" stroke="#E5E7EB" strokeWidth={1} rx={4} />
            <text x={10} y={20} fontSize="12" fontWeight="500" fill="#374151">Legend</text>
            
            {/* Expected wins legend */}
            <line x1={10} y1={35} x2={30} y2={35} stroke="#3B82F6" strokeWidth={3} strokeDasharray="4,2" />
            <text x={35} y={39} fontSize="11" fill="#3B82F6">Expected Wins</text>
            
            {/* Actual wins legend */}
            <line x1={10} y1={50} x2={30} y2={50} stroke="#059669" strokeWidth={3} />
            <text x={35} y={54} fontSize="11" fill="#059669">Actual Wins</text>

            {/* Weekly results legend */}
            <circle cx={20} cy={70} r={4} fill="#10B981" />
            <text x={30} y={74} fontSize="11" fill="#374151">Win</text>
            
            <circle cx={80} cy={70} r={4} fill="#EF4444" />
            <text x={90} y={74} fontSize="11" fill="#374151">Loss</text>

            <text x={10} y={90} fontSize="10" fill="#6B7280">Hover for details</text>
          </g>
        </svg>
      </div>
      ...
      <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
        ...
        <div>
          <div className="text-lg font-bold text-blue-600 dark:text-blue-400">
            {(chartData.reduce((sum, d) => sum + d.weeklyWinProbability, 0) / chartData.length * 100).toFixed(1)}%
          </div>
          <div className="text-sm text-gray-600 dark:text-gray-400">Avg Win Prob</div>
        </div>
        ...
        <div>
          <div className={`text-lg font-bold ${
            ...
            {chartData[chartData.length - 1]?.winLuck > 0 ? '+' : ''}{chartData[chartData.length - 1]?.winLuck.toFixed(1)}
          </div>
          <div className="text-sm text-gray-600 dark:text-gray-400">Total Luck</div>
        </div>
      </div>
    </div>
  );

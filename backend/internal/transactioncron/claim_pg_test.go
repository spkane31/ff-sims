...
// the claim query uses FOR UPDATE SKIP LOCKED, which SQLite cannot express.
func newPGTestDB(t *testing.T) *gorm.DB {
	...
	scopedDSN := testutil.NewPGSchema(t, dsn, "transactioncron_claim_test")
...
func TestClaimLeagues_OrderingLimitAndStamp(t *testing.T) {
	...
	got, err := transactioncron.ClaimLeaguesForTransactions(context.Background(), db, transactioncron.ClaimLeaguesForTransactionsParams{BatchSize: 2})
	if err != nil {
...
func TestClaimLeagues_ExcludesIneligible(t *testing.T) {
	...
	got, err := transactioncron.ClaimLeaguesForTransactions(context.Background(), db, transactioncron.ClaimLeaguesForTransactionsParams{BatchSize: 10})
	if err != nil {
		t.Fatalf("claim: %v", err)
	}

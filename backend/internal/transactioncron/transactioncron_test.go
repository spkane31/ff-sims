...
func TestRunTransactionSync_ProcessesLeaguesToCompletion(t *testing.T) {
	...
	// BatchFlushIntervalSec is long enough not to fire either — this forces
	// the assertions below to depend on fdb.RunPool's guaranteed
	// shutdown-time flush, not a size/interval trigger.
	cfg := transactioncron.Config{PoolSize: 2, RefillBatch: 1, BatchSize: 5, BatchFlushIntervalSec: 5}
...
func TestRunTransactionSync_AggregatesClaimErrors(t *testing.T) {
	...
	dfa := &activities.DataFetchActivities{DB: db, Sleeper: sleeper.NewWithBaseURL(srv.URL)}
	cfg := transactioncron.Config{PoolSize: 1, RefillBatch: 1, BatchSize: 5, BatchFlushIntervalSec: 5}
